...
class Ui_PrintingDep(object):
    def setupUi(self, MainWindow):
        ...
        #                                         r'DBQ=E:\project\المشروع\team\Upgrads\Edited.accdb;')

        # cursor = conn.cursor()
        # header=[]
        # for row in cursor.columns(table='DepartmentLevel'):
        #         header.append(str(row[3]))
        #         print(row[3])
        # self.tableWidget.setHorizontalHeaderLabels(header)
#        cur.close()   conn.close(F)
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(_translate("MainWindow", "طباعة"))
    ...
    def ReqireData(self, level, subject):
        ...
        # SELECT * from DepartmentLevel WHERE LevelID={level} and CourseID={course};
        self.Result.execute(f""" SELECT Student.StudentName, DepartmentLevel.StudentID,  Course.CourseName ,DepartmentLevel.Mark1, DepartmentLevel.Mark2, DepartmentLevel.Mark3, DepartmentLevel.Mark4
                        FROM Course INNER JOIN (Student INNER JOIN DepartmentLevel ON Student.StudentID = DepartmentLevel.StudentID) ON (Course.CourseID = Student.CourseID.Value) AND (Course.CourseID = DepartmentLevel.CourseID) WHERE CourseID={coursesNames12[0][0]} """)
        list1 = []
        ...
        self.tableWidget.setHorizontalHeaderLabels(
            ["الاسم", "رقم الجلوس", "المادة", "درجة العملي", "المشاريع", "التقارير", "بونص"])
            ...
            r'DBQ=E:\project\المشروع\team\Upgrads\Edited.accdb;')
                ...
                if int(x[3]) > 100:
                        QtWidgets.QMessageBox.about(
                            self, "خطا", "من فضلك ادخل بيانات صحيحة ")

                sql = f'UPDATE DepartmentLevel SET  DepartmentLevel.Mark1 ={x[3]}, DepartmentLevel.Mark2={x[4]}, DepartmentLevel.Mark3={x[5]}, DepartmentLevel.Mark4={x[6]} '
